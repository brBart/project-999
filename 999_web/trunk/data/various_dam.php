...
class PurchasesSummaryListDAM{
	...
	static public function getListByProduct($firstDate, $lastDate, &$total, &$totalPages, &$totalItems, $page){
		$sql = 'CALL purchases_summary_product_count(:first_date, :last_date)';
		$params = array(':first_date' => Date::dbFormat($firstDate), ':last_date' => Date::dbFormat($lastDate));
		$totalItems = DatabaseHandler::getOne($sql, $params);
		
		$totalPages = ceil($totalItems / ITEMS_PER_PAGE);
		
		$sql = 'CALL purchases_summary_total_get(:first_date, :last_date)';
		$total = DatabaseHandler::getOne($sql, $params);
		...
		$sql = 'CALL purchases_summary_product_get(:first_date, :last_date, :start_item, :items_per_page)';
		return DatabaseHandler::getAll($sql, $params);
	}
}
...
/**
 * Utility class for accessing database information regarding bonus creation and usage.
 * @package VariousDAM
 * @author Roberto Oliveros
 */
class BonusListDAM{
	/**
	 * Returns an array with data regarding bonus creation that ocurred between the provided dates.
	 *
	 * The array's fields are bar_code, manufacturer, name, quantity, percentage, created_date, expiration_date and username.
	 * If no page argument or cero is passed all the details are returned. The
	 * totalPages and totalItems arguments are necessary to return their respective values.
	 * @param string $firstDate
	 * @param string $lastDate
	 * @param integer &$totalPages
	 ...
	 * @return array
	 */
	static public function getCreatedList($firstDate, $lastDate, &$totalPages, &$totalItems, $page){
		$sql = 'CALL bonus_created_count(:first_date, :last_date)';
		$params = array(':first_date' => Date::dbFormat($firstDate), ':last_date' => Date::dbFormat($lastDate));
		$totalItems = DatabaseHandler::getOne($sql, $params);
		...
		$sql = 'CALL bonus_created_get(:first_date, :last_date, :start_item, :items_per_page)';
		return DatabaseHandler::getAll($sql, $params);
	}
	
	/**
	 * Returns an array with data regarding bonus usage that ocurred between the provided dates.
	 *
	 * The array's fields are bar_code, manufacturer, name, quantity, percentage, created_date, expiration_date and username.
	 * If no page argument or cero is passed all the details are returned. The
	 * totalPages and totalItems arguments are necessary to return their respective values.
	 ...
	 * @return array
	 */
	static public function getUsedList($firstDate, $lastDate, &$totalPages, &$totalItems, $page){
		$sql = 'CALL bonus_used_count(:first_date, :last_date)';
		$params = array(':first_date' => Date::dbFormat($firstDate), ':last_date' => Date::dbFormat($lastDate));
		$totalItems = DatabaseHandler::getOne($sql, $params);
		...
		$sql = 'CALL bonus_used_get(:first_date, :last_date, :start_item, :items_per_page)';
		return DatabaseHandler::getAll($sql, $params);
	}
}
?>