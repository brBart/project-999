<?php
/**
 * Library containing the ShowSalesRankingListCommand base class.
 * @package Command
...
/**
 * Defines functionality for the showing the sales ranking list.
 * @package Command
 * @author Roberto Oliveros
 */
class ShowSalesRankingListCommand extends Command{
	/**
	 * Execute the command.
	...
	public function execute(Request $request, SessionHelper $helper){
		...
		try{
			$list = SalesRankingList::getList($start_date, $end_date, $total_pages, $total_items, $page);
		} catch(Exception $e){
			$msg = $e->getMessage();
		...
		if($total_items > 0){	
			...
			Page::display(array('module_title' => POS_ADMIN_TITLE, 'main_menu' => 'back_link.tpl',
					...
					'second_menu' => 'none', 'content' => 'sales_ranking_list_html.tpl', 'list' => $list,
					...
					'next_link' => $next_link, 'start_date' => $start_date, 'end_date' => $end_date), 'site_html.tpl');
		}
		else {
