...
--
-- Servidor: localhost
-- Tiempo de generación: 07-03-2012 a las 12:16:36
...
CREATE TABLE IF NOT EXISTS `company` (
  ...
  `telephone` varchar(50) collate utf8_unicode_ci NOT NULL,
  `address` varchar(100) collate utf8_unicode_ci NOT NULL,
  `warehouse_name` varchar(20) collate utf8_unicode_ci NOT NULL
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci$$
...
DROP PROCEDURE IF EXISTS `bonus_used_count`$$
...
DROP PROCEDURE IF EXISTS `bonus_used_get`$$
...
DROP PROCEDURE IF EXISTS `comparison_filter_get`$$
...
BEGIN
  ...
  SELECT serial_number, resolution_number, DATE_FORMAT(resolution_date, '%d/%m/%Y') AS resolution_date,
...
BEGIN
  ...
  PREPARE statement FROM
    ...
    "SELECT serial_number, number, DATE_FORMAT(date, '%d/%m/%Y') AS date, total, state FROM invoice_transaction_log

       WHERE date BETWEEN ? AND ? ORDER BY entry_id
