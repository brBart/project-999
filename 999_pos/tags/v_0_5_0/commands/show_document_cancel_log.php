<?php
/**
 * Library containing the ShowDocumentCancelLogCommand class.
 * @package Command
 ...
 * @author Roberto Oliveros
 */
class ShowDocumentCancelLogCommand extends ShowLogCommand{
	/**
	 * Show the empty form.
	 */
	...
	protected function getList($startDate, $endDate, &$totalPages, &$totalItems, $page){
		return CancelDocumentList::getList($startDate, $endDate, $totalPages, $totalItems, $page);
	}
	...
	protected function displayList($list, $startDate, $endDate, $totalPages, $totalItems, $page, $firstItem,
		...
		Page::display(array('module_title' => INVENTORY_TITLE, 'main_menu' => 'back_link.tpl',
				'back_link' => 'index.php?cmd=show_log_menu_inventory', 'back_trace' => $back_trace,
				'second_menu' => 'none', 'content' => 'document_cancel_log_html.tpl', 'log_name' => 'Documentos Anulados',
				...
				'next_link' => $nextLink, 'print_cmd' => 'print_document_cancel_log'), 'site_html.tpl');
	}
}
?>