<?php
/**
 * Library containing the SearchWithdrawAdjustmentCommand class.
 * @package Command
...
/**
 * Implements functionality for searching a withdraw adjustment document in a given range of dates.
 * @package Command
 * @author Roberto Oliveros
 */
class SearchWithdrawAdjustmentCommand extends SearchObjectByDateCommand{
	/**
	...
	protected function getList($startDate, $endDate, &$totalPages, &$totalItems, $page){
		return WithdrawIASearch::search($startDate, $endDate, $totalPages, $totalItems, $page);
	}
	...
	protected function displayEmpty(){
		$back_trace = array('Inicio', 'Movimientos', 'Vales de Salida');
		$msg = 'No hay vales de salida en esas fechas en la base de datos.';
		Page::display(array('module_title' => OPERATIONS_TITLE, 'main_menu' => 'back_link.tpl',
				'back_link' => 'index.php?cmd=show_withdraw_adjustment_menu', 'back_trace' => $back_trace,
	...
	protected function displayList($list, $startDate, $endDate, $totalPages, $totalItems, $page, $firstItem,
		...
		Page::display(array('module_title' => OPERATIONS_TITLE, 'main_menu' => 'back_link.tpl',
				...
				'document_name' => 'Vale de Salida', 'list' => $list, 'start_date' => $startDate,
				...
				'item_link' => 'index.php?cmd=get_withdraw_adjustment&id=', 'actual_cmd' => $actualCmd),
				'site_html.tpl');
	}
}
?>