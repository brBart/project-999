<?php
/**
 * Library containing the SearchPurchaseReturnCommand class.
 * @package Command
...
/**
 * Implements functionality for searching a purchase return in a given range of dates.
 * @package Command
 * @author Roberto Oliveros
 */
class SearchPurchaseReturnCommand extends SearchObjectByDateCommand{
	/**
	...
	protected function getList($startDate, $endDate, &$totalPages, &$totalItems, $page){
		return PurchaseReturnSearch::search($startDate, $endDate, $totalPages, $totalItems, $page);
	}
	...
	protected function displayList($list, $startDate, $endDate, $totalPages, $totalItems, $page, $firstItem,
		...
		Page::display(array('module_title' => INVENTORY_TITLE, 'main_menu' => 'back_link.tpl',
				...
				'item_link' => 'index.php?cmd=get_purchase_return&id=', 'actual_cmd' => $actualCmd),
				'site_html.tpl');
	}
}
?>